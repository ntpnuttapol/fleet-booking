        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {!isMobile && (
            <div ref={userMenuRef} style={{ position: "relative" }}>
              <div onClick={() => setShowUserMenu(v => !v)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 14px 6px 6px", borderRadius: 100, border: `1.5px solid ${showUserMenu ? "#C9B8DB" : C.border}`, background: showUserMenu ? "#F4F0F8" : "#fff", cursor: "pointer", transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = "#C9B8DB"; e.currentTarget.style.background = "#F4F0F8"; }} onMouseLeave={e => { if (!showUserMenu) { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.background = "#fff"; } }}>
                <div style={{ width: 32, height: 32, background: "linear-gradient(135deg, #F5C4AB, #F0B0C4)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, fontWeight: 700 }}>{String(currentUser.name || "").substring(0, 2)}</div>
                <span style={{ fontSize: 13, fontWeight: 600, color: "#4A433C" }}>{String(currentUser.name || "")}</span>
